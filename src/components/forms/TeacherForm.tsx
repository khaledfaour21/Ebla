...
const TeacherForm = ({
    ...
    return <form className="flex flex-col gap-8" dir="rtl" onSubmit={onSubmit}>
        <h1 className="text-xl font-semibold">{type === "create" ? "انشاء استاذ جديد" : "تعديل الاستاذ"}</h1>
        <span className="text-xs text-gray-400 font-medium">معلومات المصادقة</span>
        ...
        <InputField label="الاسم الأول" name="name" 
        defaultValue={data?.name} register={register} error={errors?.name}/>

<InputField label="الاسم الأخير" name="surname" 
        defaultValue={data?.surname} register={register} error={errors?.surname}/>
...
<InputField label="Birthday" name="birthday" type="date"
        defaultValue={data?.birthday.toISOString().split("T")[0]} register={register} error={errors?.birthday}/>
  { data && (
        <InputField label="رقم" name="id" 
        ...
        <label className="text-xs text-gray-500 flex items-center gap-2 cursor-pointer">الجنس</label>
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
        <label className="text-xs text-gray-500 flex items-center gap-2 cursor-pointer">المواد</label>
