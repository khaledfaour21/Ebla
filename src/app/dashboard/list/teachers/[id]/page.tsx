...
const { sessionClamis } =await auth();
const role = (sessionClamis?.metadata as {role?: string})?.role;
        ...
        <div className="flex-1 p-4 flex flex-col gap-4 xl:flex-row">
         ...
         <div className="bg-white p-4 rounded-md text-right" dir="rtl">
            ...
            <div className="mt-4 flex gap-4 flex-wrap text-xs text-gray-500">
                <Link className="p-3 rounded-md bg-SkyLight " href={`/dashboard/list/classes?supervisorId=${"teacher2"}`}>Teacher&apos;s Classes</Link>
                <Link className="p-3 rounded-md bg-PurpleLight " href={`/dashboard/list/students?teacherId=${"teacher2"}`}>Teacher&apos;s Students</Link>
                <Link className="p-3 rounded-md bg-YellowLight " href={`/dashboard/list/lessons?teacherId=${"teacher2"}`}>Teacher&apos;s Lessons</Link>
                <Link className="p-3 rounded-md bg-pink-50 " href={`/dashboard/list/exams?teacherId=${"teacher2"}`}>Teacher&apos;s Exams</Link>
                <Link className="p-3 rounded-md bg-SkyLight " href={`/dashboard/list/assignments?teacherId=${"teacher2"}`}>Teacher&apos;s Assignments</Link>
            </div>
         </div>
         <Performance/>
         <Announcements/>
         ...
         <div className="bg-Sky py-6 px-4 rounded-md flex-1 flex gap-4 flex-row-reverse">
            ...
            <p className="text-sm text-gray-500 break-words overflow-hidden text-ellipsis">;svnvmd;sdlmvvdsvdsvsdv.</p>
            <div className="flex items-center justify-between gap-2 flex-wrap text-xs font-medium">
                ...
                <div className="w-full md:w-1/3 lg:w-full 2xl:w-1/3 flex items-center gap-2 justify-end">
                <span className="truncate">{new Intl.DateTimeFormat("ar-SY").format(teacher.birthday)}</span>
                    <Image src="/date.png" alt="" width={14} height={14}/>
                    
                </div>
                <div className="w-full md:w-1/3 lg:w-full 2xl:w-1/3 flex items-center gap-2 justify-end">
                <span className="break-words overflow-hidden text-ellipsis">{teacher.email || "-"}</span>
