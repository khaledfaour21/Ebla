...
const columns = [
    ...
    {
        header:"رقم الطالب" , accessor:"students", className:"hidden md:table-cell"
    },
...
const renderRow = (item:ParentList) => (
    <tr key={item.id} className="border-b border-gray-200 even:bg-state-50 text-sm hover:bg-PurpleLight">
        ...
        <td className="hidden md:table-cell text-right">{item.students.map(student=>student.name).join(",")}</td>
    ...
    return (
        <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0" dir="rtl">
            ...
            <div className="flex flex-row-reverse items-center justify-between">
                ...
                <h1 className="hidden md:block text-lg font-semibold text-right">كل الآباء</h1>
            </div>
            {/*  list*/}
