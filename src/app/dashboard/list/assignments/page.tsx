...
const renderRow = (item:AssignmentList) => (
    <tr key={item.id} className="border-b border-gray-200 even:bg-state-50 text-sm hover:bg-PurpleLight">
        ...
        <td className="hidden md:table-cell text-right">{item.lesson.teacher.name + " " + item.lesson.teacher.surname}</td>
        <td className="hidden md:table-cell text-right">{new Intl.DateTimeFormat("ar-SY", {timeZone: "Asia/Damascus",year: 'numeric',month: '2-digit',day: '2-digit',}).format(item.dueDate)}</td>
    ...
    return (
        <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0" dir="rtl">
            ...
            <div className="flex flex-row-reverse items-center justify-between">
                ...
                <h1 className="hidden md:block text-lg font-semibold text-right">كل المواد المفروضة</h1>
            </div>
            {/*  list*/}
